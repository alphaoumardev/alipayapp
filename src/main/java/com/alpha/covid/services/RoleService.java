...
import com.alpha.covid.structor.form.RoleForm;
...
public class RoleService extends ServiceImpl<RoleMapper, Role>
{
    ...
    public FinalResult assignMenus(Long id,  MenuVo menuIds)
    {
        ...
        if (!Objects.isNull(mids) && mids.length > 0)
        {
            ...
            for (Long mid : mids)
            {
                ...
                if (menu.getAvailable().equals(SysConstant.FORBIDDEN))
                {
    ...
    private List<RoleVo> toRoleExportVo( List<Role> roles)
    {
        ...
        roles.forEach(role ->
        {
            ...
            exportVo.setForbidden(role.getForbidden().equals(SysConstant.FORBIDDEN)? "Block" : "Active");
