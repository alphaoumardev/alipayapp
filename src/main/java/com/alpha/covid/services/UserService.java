...
import com.alpha.covid.beans.vo.RoleVo;
...
import com.alpha.covid.structor.form.UserForm;
import com.alpha.covid.structor.tools.ExcelStyleUtil;
import com.alpha.covid.structor.tools.ExcelUtil;
import com.alpha.covid.structor.tools.UserUtil;
import com.alpha.covid.structor.tools.VerifyCodeUtil;
...
public class UserService extends ServiceImpl<UserMapper,User>
{
    ...
    public IPage<User> selectUsersPages(Page<User> pages, QueryWrapper<User> wrapper)
    {
        return baseMapper.findAllUserPages(pages,wrapper);
    }

    public FinalResult login( LoginVo loginVo, HttpServletRequest request)
    {
        Long interval = customService.getKeyInterval(CustomConfig.LOGIN_VERIFY_CODE+loginVo.getVerifyCodeId());
        if(interval==null ||interval <= 3)
        {
            return FinalResult.error().message("The code has expired");
        }
        String code = customService.getLoginVerifyCode(loginVo.getVerifyCodeId());
        ...
        if (user.getForbidden().equals(SysConstant.FORBIDDEN))
        {
        ...
        if(UserUtil.matches(loginVo.getPassword(),user.getPassword()))
        {
            ...
            customService.deleteLoginVerifyCode(loginVo.getVerifyCodeId());
    ...
    {
        return this.baseMapper.resetPassword(UserUtil.encode(SysConstant.DEFAULT_PASSWORD),id);
    }
    ...
    public FinalResult assignRoles(Long id,  RoleVo roleIds)
    {
        ...
        if (!Objects.isNull(rids) && rids.length > 0)
        {
            ...
            for (Long rid : rids)
            {
                ...
                if (role.getForbidden().equals(SysConstant.FORBIDDEN))
                {
    ...
    public FinalResult getCapchaImage( String historyId) throws IOException
    {
        ...
        {
            customService.deleteLoginVerifyCode(historyId);
        }
        customService.setLoginVerifyCode(uuid,verifyCode, CustomConfig.LOGIN_VERIFY_TIME);
        //making the image
