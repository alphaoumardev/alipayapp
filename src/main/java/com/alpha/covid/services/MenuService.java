...
import com.alpha.covid.beans.vo.MenuExportVo;
import com.alpha.covid.beans.vo.MenuVo;
import com.alpha.covid.mappers.MenuMapper;
import com.alpha.covid.models.Menu;
import com.alpha.covid.structor.convertor.MenuConverter;
import com.alpha.covid.structor.form.MenuForm;
...
import com.alpha.covid.structor.tools.MenuTreeBuilderUtil;
...
public class MenuService extends ServiceImpl<MenuMapper, Menu>
{
    ...
    public List<MenuVo> findMenuTree()
    {
        ...
        {
            if(menu.getOpen().equals(SysConstant.OPEN))
            {
    ...
    public FinalResult updateMenus( MenuVo menuVo)
    {
        ...
        if(menuVo.getType().equals(SysConstant.BUTTON))
        {
        ...
        Integer newStatus = menuVo.getDisabled() ? SysConstant.FORBIDDEN : SysConstant.ACTIVE;
    ...
    public FinalResult submitMenu(MenuVo menuVo)
    {
        Menu newMenu = new Menu();
        BeanUtils.copyProperties(menuVo, newMenu);
        newMenu.setAvailable(menuVo.getDisabled() ? SysConstant.FORBIDDEN : SysConstant.ACTIVE);
    ...
    private List<MenuExportVo> toMenuExportVo( List<Menu> menus)
    {
        ...
        menus.forEach(menu ->
        {
            ...
            exportVo.setType(menu.getType().equals(SysConstant.MENU)? "Menu" : "Button");
            exportVo.setAvailable(menu.getAvailable().equals(SysConstant.FORBIDDEN) ? "Block" : "Activated");
            exportVo.setOpen(menu.getOpen().equals(SysConstant.OPEN) ? "Opened" : "Close");
