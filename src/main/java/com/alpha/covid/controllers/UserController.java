...
import com.alpha.covid.models.AliOss;
import com.alpha.covid.models.User;
import com.alpha.covid.services.OssService;
...
import com.alpha.covid.structor.tools.WordUtil;
...
@AllArgsConstructor
@RequestMapping("/user")
//@CrossOrigin(value = "/*", allowedHeaders = "*" , originPatterns = "/*")
...
public class UserController
{
    ...
    public FinalResult updateStatus(@RequestParam("id") Long id, @RequestParam("forbidden") Boolean forbidden)
    {
        ...
        User user = userService.getById(id);
        if(SysConstant.ADMIN.equals(user.getType()))
        {
            throw new BusinessException("The current user is not an administrator");
        }
        int res = userService.updateUserStatusById(id, forbidden ? SysConstant.FORBIDDEN : SysConstant.ACTIVE);
